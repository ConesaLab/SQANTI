#Script to generate a GFF3 file from SQANTI2_beta or SQANTI3 output and using a tappAS GFF3 as reference.
            ...
            for ex in allExonsGFF3:
                ...
                if ex in allExonsSQ:
                    ...
                    for exSQ in allExonsSQ:
                        if ex[0] <= exSQ[0] and ex[1] >= exSQ[1]: #Some exon starts equal or after real start and end exon ends at same point or before it is a subexon
...
def mappingFeatures(dc_SQexons, dc_SQcoding, dc_SQtransGene, dc_GFF3exonsTrans, dc_GFF3transExons, dc_GFF3, dc_GFF3coding, filename):
    ...
    for transSQ in dc_SQexons.keys():
        ...
        '''
        if infoGenomic[1] == "full-splice_match":
            if dc_GFF3.get(transGFF3):
                for values in dc_GFF3.get(transGFF3):
                    ...
                    transAnnotCorrectly = transAnnotCorrectly + 1
        '''
        ###########################
