...
def main():
	...
	parser.add_argument('isoforms', help='\tIsoforms (Fasta format)') 
	...
	if not args.mode and not args.gmap_index:
		sys.stderr.write("ERROR: for the correction of sequences a gmap_index must be specified.\n")
		sys.exit()	
...
def run(args):
	...
	## Expression information
 	...
 	# FSM classification and indels per transcript
	...
	with open(outputJuncPath, "w") as newFile:
 		with open(outputJuncPath+"_tmp", "r") as oldFile:
				...
				for line in oldFile:
	 				split_l = (line.split("\t"))
					rts_status = "FALSE"
					if ":".join([split_l[0], split_l[2],split_l[4],split_l[5]]) in RTSjunc_list: 
						rts_status = "TRUE"
	...
	sys.stdout.write("\nSQANTI report successfully generated!\n")
